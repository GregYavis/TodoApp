...
class RegisterPage(FormView):
    ...
    # For redirecting if we are loged in
...
class TaskList(LoginRequiredMixin, ListView):
    ...
    # To get tasks (or others data) that current user owns
    def get_context_data(self, *, object_list=None, **kwargs):
        context = super().get_context_data(**kwargs)
        context['tasks'] = context['tasks'].filter(user=self.request.user)
        context['count'] = context['tasks'].filter(complete_flag=False).count()
...
class TaskCreate(LoginRequiredMixin, CreateView):
    model = Task
    fields = ['title', 'description', 'complete_flag']
    success_url = reverse_lazy('tasks')
...
class TaskUpdate(LoginRequiredMixin, UpdateView):
    ...
    fields = ['title', 'description', 'complete_flag']
    success_url = reverse_lazy('tasks')
    #template_name = 'base_app/task-update-form.html'
